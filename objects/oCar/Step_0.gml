...
var _obs_front = collision_rectangle(_x1 + 5, _y1 + 5, _x2 + 5, _y2 + 5, [oPlayer, oCar, oBus], false, true);
...
if (_other_car_in_intersection != noone) {
	if (_car_ahead_intersection == id)
	{
		if (direction == 0)
		{
			if (Get_Distance(x, x - 62) < Get_Distance(_car_ahead_intersection.y, _car_ahead_intersection.y - 62))
			{
				_should_stop = false;
			}
			else
		...
		if (direction == 90)
		{
			if (Get_Distance(y, y - 62) < Get_Distance(_car_ahead_intersection.x, _car_ahead_intersection.x - 62))
			{
				_should_stop = false;
			}
			else
